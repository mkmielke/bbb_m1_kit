...
/******************************************************************************/
unsigned char crc_8( char* data, unsigned char poly, unsigned char init_val );
 ...
 * Parameters:
 *   data     - array of input data. MUST BE NULL TERMINATED
...
float sht31_get_humidity( void )
{
  ...
  r_buff[5] = '\0';
  if ( crc_8( r_buff + 3, 0x31, 0xFF ) != hum_crc )
  {
    return -5;
  }
...
float sht31_get_temperature( void )
{
  ...
  // recalculate the temperature check sum and compare with the received value
  r_buff[2] = '\0';
  if ( crc_8( r_buff, 0x31, 0xFF ) != temp_crc )
  {
    return -5;
  }
